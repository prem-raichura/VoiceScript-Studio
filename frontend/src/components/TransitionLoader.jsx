...
/* ════════════════════════════════════════════
   ...
   w-full lg:w-80, border-r, bg-slate-50
   ════════════════════════════════════════════ */
  ...
  return (
    <aside className="relative z-20 w-full lg:w-80 border-r border-slate-200 bg-slate-50 shadow-sm flex flex-col lg:h-full flex-shrink-0">
      ...
      <div className="flex-1 p-5 overflow-y-auto scrollbar-thin space-y-3">
        ...
        {[0, 1].map(i => (
          <div key={i} className="w-full p-3.5 rounded-xl border border-slate-200 bg-slate-50 flex items-start gap-3">
...
function MainSkeleton() {
  return (
    <main className="relative z-10 flex-1 h-full overflow-y-auto scrollbar-thin flex flex-col">
      ...
      <div className="h-16 border-b border-slate-200 bg-slate-50 items-center justify-between px-6 flex-shrink-0 hidden lg:flex">
      ...
      <div className="p-4 lg:p-6 grid grid-cols-1 xl:grid-cols-12 gap-6 w-full max-w-7xl mx-auto">
        ...
        <section className="xl:col-span-8 flex flex-col gap-4">
          ...
          {/* Translation section — rounded-2xl border-slate-300 bg-teal-100/70 p-5 */}
          <div className="rounded-2xl border border-slate-300 bg-teal-100/40 p-5 space-y-3">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Bone className="w-5 h-5 rounded-lg flex-shrink-0" />
                <Bone className="h-[10px] w-24 rounded-md" />
...
export default function TransitionLoader({ onDone }) {
  ...
  return (
    ...
    >
      <div className="min-h-screen lg:h-screen w-full bg-transparent relative overflow-hidden text-slate-900 flex flex-col lg:flex-row">
        <SidebarSkeleton />
        <MainSkeleton />
      </div>
    </div>
