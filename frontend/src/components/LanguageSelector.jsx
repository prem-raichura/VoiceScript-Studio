...
function Select({ value, onChange, options, disabled, label }) {
  return (
    <div className="flex-1 min-w-0">
      <label className="block text-[10px] font-bold text-brand-400 mb-1.5 uppercase tracking-widest">{label}</label>
      <div className="relative">
        ...
        <div className="pointer-events-none absolute inset-y-0 right-2.5 flex items-center text-brand-300">
          <ChevronDown size={14} />
        </div>
      </div>
    </div>
    ...
    <div className="flex items-end gap-3">
      ...
      <div className="flex-shrink-0 mb-3 flex flex-col items-center gap-0.5">
        <div className="w-7 h-7 rounded-full bg-brand-50 border border-brand-100 flex items-center justify-center">
          <Globe size={12} className="text-brand-400 animate-rotate-slow" />
        </div>
      </div>
      <Select
        label="Translate to"
